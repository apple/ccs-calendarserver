...
gss_cred_id_t global_delegated_cred_handle = NULL;
...
int authenticate_gss_client_clean(gss_client_state *state)
{
    ...
    if (global_delegated_cred_handle)
    {
        maj_stat = gss_release_cred(&min_stat, &global_delegated_cred_handle);
	global_delegated_cred_handle = NULL;
    }
    return ret;
}
...
int authenticate_gss_client_step(gss_client_state* state, const char* challenge)
{
    ...
    if (global_delegated_cred_handle && (state->gss_flags & GSS_C_DELEG_FLAG))
    {
	cred_handle = global_delegated_cred_handle;
    }
    ...
    // gss_inquire_context when gss_init_sec_context is passed with delegated credential
