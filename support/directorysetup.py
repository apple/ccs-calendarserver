...
def addUser(admin_user, admin_pswd, node, computername, username):
    ...
    cmd = "dscl -u %s -P %s %s -create /Users/%s \"dsAttrTypeStandard:CalendarPrincipalURI\" \"%s\"" % (admin_user, admin_pswd, node, username, servicetag,)
    print cmd
    os.system(cmd)
...
def removeUser(admin_user, admin_pswd, node, computername, username):
    cmd = "dscl -u %s -P %s %s -delete /Users/%s \"dsAttrTypeStandard:CalendarPrincipalURI\"" % (admin_user, admin_pswd, node, username,)
    print cmd
    os.system(cmd)
